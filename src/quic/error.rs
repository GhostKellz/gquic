...
pub enum ProtocolError {
    ...
    #[error("Packet number out of range")]
    PacketNumberOutOfRange,
    
    #[error("Duplicate packet number")]
    DuplicatePacketNumber,
    
    #[error("Invalid stream ID: {0}")]
    InvalidStreamId(u64),
    
    #[error("Frame not allowed in packet type: {frame_type} in {packet_type:?}")]
    FrameNotAllowed { frame_type: u8, packet_type: String },
}
