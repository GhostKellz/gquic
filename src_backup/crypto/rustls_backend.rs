...
use ring::{aead, hkdf, rand::{self, SecureRandom}, signature::{self, KeyPair as RingKeyPair}};
...
impl CryptoBackend for RustlsBackend {
    ...
    fn encrypt_aead(&self, key: &[u8], nonce: &[u8], _aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    fn decrypt_aead(&self, key: &[u8], nonce: &[u8], _aad: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>> {
